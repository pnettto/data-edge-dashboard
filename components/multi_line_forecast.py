...
def _compute_forecast_series(df: pd.DataFrame, x_field: str, y_field: str, periods: int, freq: str, model_params: dict) -> pd.DataFrame:
...
def _render_chart_content(df, x_field, y_field, category_field, forecast_periods, config):
    ...
    for series_name, group in df.groupby(category_field, sort=False):
        ...
        if len(group_sorted) >= 2:
            ...
            forecast = _compute_forecast_series(group_sorted, x_field, y_field, forecast_periods, freq, model_params)
            forecast_df = pd.DataFrame({x_field: forecast['ds'], y_field: forecast['yhat'], category_field: series_name, "type": ["Forecast"] * len(forecast)})
            forecast_frames.append(forecast_df)

            # --- Add connector line between last actual and first forecasted point for this series ---
    ...
    # To make forecasted lines dashed but keep the same color as the original line,
    # we layer three charts: one for Actual (solid), one for Forecast (dashed), one for Connector (dashed), all colored by series.
